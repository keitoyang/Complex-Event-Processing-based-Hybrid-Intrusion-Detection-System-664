	...
	public int startListener() throws Exception {// type 1 server, type 0 client
		if (type) {// server & receiver
			...
			while (true) {
				...
				Thread.sleep(5000);
				file = new byte[10000000];
				System.out.println(is.available());
				byte b;
				int br = 0,pp=0;
				do {
					br = is.read(file, pp, file.length - pp);
					if(br>0)
						pp += br;
				} while(br > -1);
				temp.write(file);
				...
				System.out.println("Update Recieved Successfully!!");
	...
	public synchronized static void addDevice(Device host) {
		for (Device device : devices) {
			if (host.getDeviceID().equals(device.getDeviceID())) {
				host.disk_read_diff = (device.getdisk_usage_read() - host.getdisk_usage_read())/5.0;
				host.disk_write_diff = (device.getdisk_usage_write() - host.getdisk_usage_write())/5.0;
